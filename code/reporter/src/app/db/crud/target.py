...
class CrudTarget(CrudBase):
    ...
    def update_scan_status(self, target: Target):
        ...
        # ! MongoDB stores time in UTC, when we store time with Go it stores in UTC
        # ! However, when we store time with Python it stores in local time
        # ! but mongodb assumes it's UTC
        ...
        if target.failure_reason:
            to_update["failure_reason"] = target.failure_reason
