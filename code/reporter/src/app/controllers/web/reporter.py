...
class Reporter(object):
    ...
    def run(self):
        ...
        try:
            ...
            output_dir = os.path.join(
                ...
                # "/app/iva/scanner/local_temp/reports/",
    ...
    def add_alert(self, alert):
        try:
            ...
            vulnerability_title = alert.get("vulnerability_title")
            ...
            severity_str = SeverityIndex.get_string(alert["severity"])
            url = alert.get("web_details", {}).get("url")
            param = alert.get("web_details", {}).get("param")
            instances = alert.get("web_details", {}).get("instances")
            remediation = alert.get("remediation")
            reference = alert.get("reference")

            payload = alert.get("web_details", {}).get("payload")
            evidence = alert.get("evidence")
            other = alert.get("web_details", {}).get("other")
            cvss_score = alert.get("classification", {}).get("cvss_score")

            p1 = self.document.add_paragraph()
            p1.paragraph_format.page_break_before = True

            self.document.add_heading(f"{alert_index}. {vulnerability_title}", level=2)
            self.document.add_paragraph()
            ...
            finding = common_utils.smart_str(finding)
            self.document.add_paragraph(common_utils.smart_str(finding))

            if url:
                url = common_utils.smart_str(url)
                url = url.replace("\00", "[NULL_BYTE]")
                self.document.add_heading("URL\t" + tabs + ": ", level=2)
                self.document.add_paragraph(url)

            if param:
                param = common_utils.smart_str(param)
                self.document.add_heading("Parameter" + tabs + ": ", level=2)
                self.document.add_paragraph(param)

            if instances:
                param = common_utils.smart_str(instances)
                self.document.add_heading("Instances" + tabs + ": ", level=2)
                self.document.add_paragraph(instances)

            if payload:
                payload = common_utils.smart_str(payload)
                payload = payload.replace("\00", "[NULL_BYTE]")

                self.document.add_heading("Payload" + tabs + ": ", level=2)

                self.document.add_paragraph(payload)

            if evidence:
                evidence = common_utils.smart_str(evidence)
                evidence = evidence.replace("\00", "[NULL_BYTE]")

                self.document.add_heading("Evidence" + tabs + ": ", level=2)
                self.document.add_paragraph(evidence)

            if other:
                other = common_utils.smart_str(other)
                other = other.replace("\00", "[NULL]")

                self.document.add_heading("Additional Details\t\t: ", level=2)
                self.document.add_paragraph(other)
