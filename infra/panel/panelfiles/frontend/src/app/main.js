...
export function virtualFormSubmit(path, params, method) {
  ...
  params[csrfData.CSRFName] = csrfData.CSRFToken;
...
ready(function () {
  ...
  const alertBoxCloseButtons =
    document.getElementsByClassName("alert-box-close");
  ...
  selectAllRowsCheckboxes.forEach(function (checkbox) {
    checkbox.addEventListener("click", function (e) {
      ...
      const checkboxes = currentTable.querySelectorAll(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll("form").forEach(function (form) {
    ...
    csrfInput.setAttribute("name", csrfData.CSRFName);
    csrfInput.setAttribute("value", csrfData.CSRFToken);
...
export function requestWithCSRFToken(url, options = {}) {
  ...
  options.headers["X-CSRF-Token"] = csrfData.CSRFToken;
  options.headers["X-Requested-With"] = "XMLHttpRequest";

  // Add CSRF token to POST request body
  if (options.method.toUpperCase() === "POST") {
    let data = options.body || new FormData();
    if (typeof data === "string" && !data.includes(csrfData.CSRFName)) {
      data += `&${csrfData.CSRFName}=${csrfData.CSRFToken}`;
    } else if (data instanceof FormData) {
      if (!data.has(csrfData.CSRFName)) {
        data.append(csrfData.CSRFName, csrfData.CSRFToken);
      }
    }
    options.body = data;
  }
...
ready(function () {
  ...
  sidebarLinks.forEach((link) => {
    if (link.hasAttribute("data-toggle")) {
      ...
      let ul = document.getElementById(linkId);
      let sublink = document.querySelectorAll("#" + linkId + " a");
      sublink.forEach((sub_link) => {
        if (sub_link.href === url) {
          ul.classList.add("show");
          link.classList.remove("collapsed");
          link.classList.add("active");
          // Add 'active' class to the parent of the link
          sub_link.classList.add("active");
        }
      });
    }
    if (link.href === url) {
...
$(document).ready(function () {
  ...
  $.ajaxPrefilter(function (options, originalOptions, jqXHR) {
    if (
      ...
      options.data.indexOf("" + csrfData.CSRFName + "") < 0
    ) {
      // initialize `data` to empty string if it does not exist
      options.data = options.data || "";
      // add leading ampersand if `data` is non-empty
      options.data += options.data ? "&" : "";
      options.data += "" + csrfData.CSRFName + "=" + csrfData.CSRFToken + "";
    }
  });
});
