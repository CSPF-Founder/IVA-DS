import { ready, virtualFormSubmit } from "./main.js";
...
ready(function () {
  ...
  const pageMeta = JSON.parse(
    document.getElementById("scan-results-meta").textContent
  );

  let alert_chart_data = pageMeta.VulnDistribution;
...
ready(function () {
  ...
  exportButton.addEventListener("click", function (e) {
    virtualFormSubmit("/scan-results/export-alerts", {});
  });
});

ready(function () {
  document.querySelectorAll(".update-alert-status").forEach((button) => {
    button.addEventListener("click", function (e) {
      ...
      form.method = "POST";
      form.action = `/targets/${targetId}/alerts/${alertId}/status`;
      form.innerHTML = `<input type="hidden" name="${csrfData.CSRFName}" value="${csrfData.CSRFToken}" />`;
      form.innerHTML += `<input type="hidden" name="flag" value="${flag}" />`;
      form.innerHTML += `<input type="hidden" name="alert_id" value="${alertId}" />`;
      form.innerHTML += `<input type="hidden" name="target_id" value="${targetId}" />`;
