...
ready(function () {
  ...
  let todayDTElem = document.getElementById("today_date");
  if (!todayDTElem) {
    return;
  }

  let todayDateVal = todayDTElem.value;
  if (!todayDateVal) {
    return;
  }

  var today_date = new Date(todayDateVal);
  ...
  addBugTrack.addEventListener("submit", function (e) {
    ...
    requestWithCSRFToken("/bug-track/add", {
      method: "POST",
      body: formData,
    })
      .then((response) =>
